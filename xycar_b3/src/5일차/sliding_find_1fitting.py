...
warp_src  = np.array([
    [120-warpx_margin, 320-warpy_margin],  
    [20-warpx_margin, 360+warpy_margin],
    [540+warpx_margin, 320-warpy_margin],
...
def warp_process_image(img):
    ...
    if len(lx) != 0:
        lfit1 = np.polyfit(np.array(ly),np.array(lx),1)

    else:
        lfit1 = [0, 0]
    if len(rx) != 0:
        rfit1 = np.polyfit(np.array(ry),np.array(rx),1)
    else:
        rfit1 = [0,0]
 

    linear_model_l=np.poly1d(lfit1)
    linear_model_r=np.poly1d(rfit1)
    x_s=np.arange(0,640)
    ...
    return lfit1, rfit1, num_l, num_r, lx, ly, rx, ry
...
def draw_lane(image, warp_img, Minv, left_fit, right_fit, cnt_L, cnt_R, lx, ly, rx, ry):
    ...
    start_point = []
    if left_pt != 0:
        start_point.append(left_pt)
    if right_pt != 0 :
        start_point.append(right_pt)
        
    mid_point = np.mean(start_point)
    ...
    if len(lx) != 0:    
        ...
        if lx[0] > lx[-1]:
            ...
            LH_RAD = np.arctan(left_fit[0])+val
            #if (LH_RAD<0.2) : LH_RAD = 0
            LH_RAD = LH_RAD * -200/1.5
        else:

            LH_RAD = 3.14-np.arctan(left_fit[0])-val
            #if (LH_RAD<0.2) : LH_RAD = 0
            LH_RAD = LH_RAD * 200/1.5
    else :
        L_H = 0
    ...
    if len(rx) != 0:
        if rx[0] > rx[-1]:
            ...
            RH_RAD = (np.arctan(right_fit[0]))-val
            #if (RH_RAD<0.2) : RH_RAD = 0
            RH_RAD = RH_RAD * -200/1.5
            
        else:

            RH_RAD = 3.14-np.arctan(right_fit[0])+val
            #if (RH_RAD<0.2) : RH_RAD = 0
            RH_RAD = RH_RAD * 200/1.5
    ...
    if (abs(LH_RAD)>abs(RH_RAD)):
        avr = LH_RAD
    else : avr = RH_RAD

    print("right_fit[0]", right_fit[0], "np.arctan(right_fit[0])",np.arctan(right_fit[0]),"LH_RAD",LH_RAD,"RH_RAD",RH_RAD,"avr",avr,"val",val)
