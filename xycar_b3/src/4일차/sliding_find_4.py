...
warp_src  = np.array([
    [140-warpx_margin, 320-warpy_margin],  
...
def warp_process_image(img):
    ...
    for idx in range(1, 3):
        arry = []
        t = 0
        for i in range(n, histogram.shape[0]):
            if histogram[i] >= 8000:
                arry.append(i)
                t = 1
            elif t == 1:
    ...
    temp = chk_arry.pop()
    if len(temp) != 0: chk_arry.append(temp)
    if len(chk_arry) == 1:
    	for chk in chk_arry:
            if len(chk) != 0:
                if prev_l > prev_r:
                    leftx_current = np.argmax(histogram[chk[0]:chk[-1]])
                else:
                    rightx_current = np.argmax(histogram[chk[0]:chk[-1]])
    #print(leftx_current, rightx_current)
    #cv2.imshow("img", img)
...
def draw_lane(image, warp_img, Minv, left_fit, right_fit, cnt_L, cnt_R):
    ...
    '''
    L_H = math.sqrt((left_fitx[320]*x_mpp - (left_fitx[0]*x_mpp + left_fitx[480]*x_mpp)/2)**2 + (320*y_mpp - 320*y_mpp)**2)
    R_H = math.sqrt((right_fitx[320]*x_mpp - (right_fitx[0]*x_mpp + right_fitx[480]*x_mpp)/2)**2 + (320*y_mpp - 320*y_mpp)**2)
    L_W = math.sqrt((left_fitx[0]*x_mpp - left_fitx[480]*x_mpp)**2 + (480*y_mpp)**2)
    R_W = math.sqrt((right_fitx[0]*x_mpp - right_fitx[480]*x_mpp)**2 + (480*y_mpp)**2)
    ...
    error = (480 - (left_fitx[-1] +right_fitx[-1])/2) * x_mpp
