...
warp_src  = np.array([
    [140-warpx_margin, 300-warpy_margin],  
    [20-warpx_margin, 360+warpy_margin],
    [500+warpx_margin, 300-warpy_margin],
...
def warp_image(img, src, dst, size):
    ...
    #cv2.rectangle(warp_img, (220, 370), (410, 480),(255, 255, 255),-1)
    ...
    for i in range(2):
        ...
        for j in range(n, histogram.shape[0]):
            ...
            if histogram[j] >= 10000:
                ...
                break
        n = t
        if len(arry) != 0: chk_arry.append(arry)
    ...
    if len(chk_arry) == 2:
        ...
        for chk in chk_arry:
            ...
            if len(chk_arry) == 1 and prev_l > prev_r:
                ...
                rightx_current = np.argmax(histogram[(chk[-1]+ 1):]) + np.int(midpoint * 2)
            elif len(chk_arry) == 1 and prev_r > prev_l:
                leftx_current = np.argmax(histogram[:chk[0]-1])
                rightx_current = np.argmax(histogram[chk[0]:]) + np.int(chk[0])
            else:
                max = np.argmax(histogram[chk[0]:chk[-1]])
                print("same", max)
                leftx_current = np.argmax(histogram[:chk[-1]])
                rightx_current = np.argmax(histogram[(chk[-1]+ 1):])
...
def draw_lane(image, warp_img, Minv, left_fit, right_fit, cnt_L, cnt_R, lx, ly, rx, ry):
    ...
    y_mpp = 1 # 0.003 * 480
    # focal size = 36mm
    # format size (lens size) = 32 x 32 mm
    # distance of camera object 5cm ~ 100m (100m select)
    # allowable coverage = 100%
    # resolution = 2mega-pixels
    # computed coverage (maximum object size) = 88.89m
    # computed pixel size = 0.063
    # manual markin = 2x = 0.126
    # subpixel = 1/20 = 0.003
    # dot size = 8x = 0.503
    # camera spec = http://www.camera-module.com/product/2mpcameramodule/1080p-camera-omnivision-ov2710-cmos-sensor.html
    # calculator = https://www.photomodeler.com/pm-support/field-of-view-calculator/
    ...
    if len(lx) != 0:
        ...
        if lx[0] > lx[-1]:
            LH_RAD = (0.57 / (L_RAD + val)) * -1
        else:
            LH_RAD = (0.57 / (L_RAD - val))
    else :
        L_H = 0
        L_W = 0
        L_RAD = 999999999
    ...
    if len(rx) != 0:
        ...
        if right_fitx[0] > right_fitx[-1]:
            RH_RAD = (0.57 / (R_RAD - val)) * -1
        else:
            RH_RAD = (0.57 / (R_RAD + val))
