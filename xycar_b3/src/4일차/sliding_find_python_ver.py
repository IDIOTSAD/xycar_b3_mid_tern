...
warp_src  = np.array([
    [50-warpx_margin, 350-warpy_margin],  
    [40-warpx_margin, 400+warpy_margin],
    [590+warpx_margin, 350-warpy_margin],
    [600+warpx_margin, 400+warpy_margin]
], dtype=np.float32)
'''
...
if calibrated:
    ...
    dist = np.array([0., 0., 0.000, 0.000, 0.000000])
    cal_mtx, cal_roi = cv2.getOptimalNewCameraMatrix(mtx, dist, (Width, Height), 1, (Width, Height))
    ...
    if len(chk_arry) == 2:
        ...
        for chk in chk_arry:
            ...
            if len(chk_arry) == 1 and prev_l > prev_r:
                ...
                rightx_current = np.argmax(histogram[(chk[-1]+ 1):]) + np.int(chk[-1] + 1)
...
def start():
    ...
    while cap.isOpened():
        ...
        #avr = draw_lane(image, warp_img, Minv, left_fit, right_fit, (cnt_left), (cnt_right), lx, ly, rx, ry)
